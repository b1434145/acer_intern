...
from .dbs.traffic_light import TRAFFIC_LIGHT
...
class CornerNet_Squeeze_traffic_light(Base):
    def __init__(self):
        from .test.cornernet import cornernet_inference
        from .models.CornerNet_Squeeze_traffic_light import model

        cfg_path = get_file_path("..", "configs", "CornerNet_Squeeze_traffic_light.json")
        model_path = get_file_path("..", 'cache', "nnet", "CornerNet_Squeeze_traffic_light_v4", "CornerNet_Squeeze_traffic_light_260000.pkl")

        cfg_sys, cfg_db = load_cfg(cfg_path)
        sys_cfg = SystemConfig().update_config(cfg_sys)
        traffic_light = TRAFFIC_LIGHT(cfg_db)

        cornernet = load_nnet(sys_cfg, model())
        super(CornerNet_Squeeze_traffic_light, self).__init__(traffic_light, cornernet, cornernet_inference, model=model_path)
