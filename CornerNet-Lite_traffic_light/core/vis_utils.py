...
def draw_bboxes(image, bboxes, name, font_size=0.5, thresh=0.5, colors=None):
    ...
    if len(bboxes['traffic light'][keep_inds]) == 0:
        with open('/home/arno/Desktop/detection-results/'+str(name)+'txt','w') as output:
            output.write('')
        

    for bbox in bboxes['traffic light'][keep_inds]:
        print('trafficlight',bbox[4],int(bbox[0]/960*1920),int(bbox[1]/540*1080),int(bbox[2]/960*1920),int(bbox[3]/540*1080),file = open('/home/arno/Desktop/detection-results/'+str(name)+'txt','a'))
        # print('trafficlight',bbox[4],int(bbox[0]),int(bbox[1]),int(bbox[2]),int(bbox[3]),file = open('/home/arno/Desktop/detection-results/'+str(name)+'txt','a'))
        ...
        cv2.rectangle(image,
            ...
            # a = str('traffic li'+' '+str(obj_struct['bbox'][0])+' '+str(obj_struct['bbox'][1])+' '+str(obj_struct['bbox'][2])+' '+str(obj_struct['bbox'][3])
            
            # print(bboxes[cat_name][keep_inds])
